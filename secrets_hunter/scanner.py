...
def process_file(
    ...
    matched_rule = __scan_file_extension(file_extension, rules)
    ...
    matched_rule = __scan_file_name(file_name, rules)
    if matched_rule is not None:
        __add_discovery(
            discoveries_queue,
            file_path,
    ...
    matched_rule = __scan_file_path(file_path, rules)
    if matched_rule is not None:
        __add_discovery(
            discoveries_queue,
            file_path,
    ...
    try:
        ...
        logger.exception("Failed to open {0} file".format(file_path))
        return

    try:
        for match_data, line_nb, matched_rule in __scan_file_content(
            file_fd, logger, rules
        ):
            __add_discovery(
                discoveries_queue,
                file_path,
        ...
        logger.exception("Failed to read {0} file".format(file_path))
    finally:
        file_fd.close()
...
def __add_discovery(
    ...
    for excluded_pattern in rules.excluded_patterns:
...
def __scan_file_extension(
    file_extension: str, rules: RulesParser
    ...
    for rule in rules.extension_rules:
        if rule[3] != EMPTY_REGEX:  # Test for `regex` attribute
...
def __scan_file_name(
    file_name: str, rules: RulesParser
) -> Optional[Tuple[str, str, str, Pattern[str], str, str]]:
    """Scan the file name.
    ...
    for rule in rules.filename_rules:
        if rule[3] != EMPTY_REGEX:  # Test for `regex` attribute
...
def __scan_file_path(
    file_path: str, rules: RulesParser
) -> Optional[Tuple[str, str, str, Pattern[str], str, str]]:
    """Scan the file path.
...
def __scan_file_content(
    file_fd: BinaryIO, logger: logging.Logger, rules: RulesParser
) -> Generator[
